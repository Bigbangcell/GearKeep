...
export function ItemTooltip({ item, anchorRect }: ItemTooltipProps) {
  const viewportHeight = window.innerHeight;
  const showAbove = anchorRect.top > viewportHeight / 2;

  const tooltipStyle: React.CSSProperties = {
    position: 'fixed',
    left: anchorRect.left,
    width: Math.min(anchorRect.width * 2, 320),
    top: showAbove ? anchorRect.top - 10 : anchorRect.bottom + 10,
